  ...
  return (
    <section className="relative flex items-center justify-center overflow-hidden bg-dark" style={{ minHeight: 'clamp(85vh, 100vh, 100vh)' }}>
      ...
      <div className="relative z-10 text-center container-fluid max-w-fluid mx-auto px-4 sm:px-6" style={{ paddingTop: 'clamp(5rem, 12vw, 8rem)', paddingBottom: 'clamp(2rem, 5vw, 4rem)' }}>
        <div className="mb-[clamp(2rem,5vw,2.5rem)] animate-fade-in" style={{ animationDelay: '0.2s' }}>
        ...
        <div className="animate-fade-in-up px-4" style={{ animationDelay: '0.6s' }}>
          ...
          >
            <span className={`inline-block transition-transform duration-300 ${isArabic ? 'group-hover:-translate-x-1' : 'group-hover:translate-x-1'}`}>{t('hero.cta')}</span>
            <svg className={`w-[clamp(1rem,1.2vw,1rem)] h-[clamp(1rem,1.2vw,1rem)] transition-transform duration-300 ${isArabic ? 'rotate-180 group-hover:-translate-x-1' : 'group-hover:translate-x-1'}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
